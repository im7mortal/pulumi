...
for i in  "github.com/pulumi/pulumi-dotnet dotnet v3.66.2"; do
  ...
  (
    ...
    for j in "darwin" "linux" "windows .exe"; do
      ...
      for k in "amd64 x64" "arm64 arm64"; do
        ...
        ARCHIVE="pulumi-analyzer-policy-${PULUMI_LANG}-${TAG}-${DIST_OS}-${DIST_ARCH}"
        ...
        tar -xzvf "${ARCHIVE}.tar.gz" -C "${OUTDIR}" "pulumi-analyzer-policy-${PULUMI_LANG}${DIST_EXT}"
      done
    done
  )
done
