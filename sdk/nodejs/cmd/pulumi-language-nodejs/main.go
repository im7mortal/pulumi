...
func main() {
	...
	rc.Flag.String("tsconfig", "",
		"[obsolete] Path to tsconfig.json to use")
	rc.Flag.String("nodeargs", "", "[obsolete] Arguments for the Node process")
	rc.Flag.String("packagemanager", "", "[obsolete] Packagemanager to use (auto, npm, yarn or pnpm)")
	err = rc.Flag.Parse(os.Args[1:])
	if err != nil {
		cmdutil.Exit(err)
	}
